...
<template>
  <div class="h-dvh w-full flex flex-col justify-center items-start gap-8">
    <h1 class="text-2xl lg:text-5xl w-full text-center lg:text-left">meet the team</h1>
    <div class="h-3/4 lg:h-5/9 w-full flex flex-col lg:flex-row justify-center items-center">
      <div class="overflow-x-hidden lg:overflow-x-auto w-full lg:w-2/12 h-1/4 lg:h-full flex lg:flex-col flex-row justify-between items-center relative">
        <svg
          width="41"
          height="85"
          viewBox="0 0 41 85"
        ...
        <div
          ...
          class="h-full w-full min-w-full lg:w-full lg:h-1/5 rounded-tr-xl rounded-tl-xl lg:rounded-tr-none lg:rounded-br-xl overflow-clip relative transition-all"
          :class="index == activeMember ? 'opacity-100' : 'opacity-20'"
      ...
      <div class="hidden lg:block w-5/12 2xl:w-6/12 h-full relative rounded-l-xl overflow-clip">
        <img
          class="w-full h-full object-cover object-center transition-all"
          :src="team[activeMember].img"
        />
        <h2
          class="text-9xl font-secondary font-bold absolute -bottom-4 right-2 transition-all"
        >
          {{ team[activeMember].nickname }}
        </h2>
      </div>
      <div
        class="w-full lg:w-4/12 2xl:w-3/12 h-3/4 lg:h-full p-6 rounded-bl-xl lg:rounded-bl-none rounded-br-xl lg:rounded-r-xl bg-white flex flex-col gap-3 justify-start lg:justify-center items-start overflow-y-scroll lg:overflow-y-clip"
      >
        <div>
          <h3 class="text-lg lg:text-2xl text-black font-primary mb-1 transition-all">
            {{ team[activeMember].fullname.toLowerCase() }}
          </h3>
          <p class="text-md lg:text-xl text-black font-secondary italic font-bold transition-all">
            {{ team[activeMember].role }}
          </p>
        </div>
        <p class="text-sm lg:text-md text-black font-secondary text-justify transition-all">
          {{ team[activeMember].info }}
        </p>
        <div
          class="grid grid-cols-2 w-full gap-2 justify-items-center content-center"
        >
          <p class="text-center text-sm lg:text-md text-black font-secondary p-2 w-full">
            favorite food item
          </p>
          <p
        ...
        <p class="text-sm lg:text-md text-black font-secondary italic text-center transition-all">
